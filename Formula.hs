...
bv f = case f of
  {
   Atom w ts -> [] ;
   And f1 f2 -> bv f1 `union` bv f2;
   Or f1 f2  -> bv f1 `union` bv f2;
   Imp f1 f2 -> bv f1 `union` bv f2;
   Bot       -> [];
   All x g   -> [x] `union` bv g;
   Ex x g    -> [x] `union` bv g
  }
...
parseFun = do { 
                w <- identifier;
                args <- bracket (char '(') (sepby (parse (token (char ','))) parseTerm) (char ')');
                return (Fun w args)
              }
               
parseTerms :: Parser [Term]
parseTerms = inbrackets (sepby (parse (token (char ','))) parseTerm) 
             `plus`
             return []
...
atomform :: Parser Formula
-- atomform = do { w <- word ; return (Atom w) }
atomform = do { 
                w <- identifier ; 
                if not (w `elem` botstrings) 
                then do {ts <- parseTerms []; return (Atom w ts)} 
                else zero
              }
...
basicform :: Parser Formula
basicform = atomform `plus` botform

-- unary logical operators

negation, allquantifier, exquantifier :: Parser (Formula -> Formula)
...
inform :: Parser Formula
inform = logop1form `plusst` (insubform `chainr1` logop2)

insubform :: Parser Formula
insubform = inbrackets inform `plusst` atomform

logop1form :: Parser Formula
logop1form = token (do {f<- logop1; sf <- insubform; return (f sf)})


-- arbitrary infix formulas with priorities
                 
informp :: Parser Formula
informp = logop1formp `plusst` (insubformp `chainp` logop2p)

insubformp :: Parser Formula
insubformp = inbrackets informp `plusst` atomform

logop1formp :: Parser Formula
logop1formp = token (do {f<- logop1; sf <- insubformp; return (f sf)})
...
parseFormula0 :: Parser Formula
parseFormula0 = p1 (1,3) logoppref logopinfix

-- Activating bindings e.g.  
-- All "x" (Atom "P" [Const "x"])  |->  All "x" (Atom "P" [Var "x"])  

activateTerm :: [String] -> Term -> Term
activateTerm xs t = 
    case t of
      {
        Var x    -> t ;
        Const x  -> if x `elem` xs then Var x else t ;
        Fun f ts -> Fun f (map (activateTerm xs) ts)
      }

activateFormula :: [String] -> Formula -> Formula
activateFormula xs f = 
    case f of
      {
        Atom p ts -> Atom p (map (activateTerm xs) ts) ;
        And f g   -> And (activateFormula xs f) (activateFormula xs g) ;
        Or f g    -> Or  (activateFormula xs f) (activateFormula xs g) ;
        Imp f g   -> Imp (activateFormula xs f) (activateFormula xs g) ;     
        Bot       -> Bot ;
        All x f   -> All x (activateFormula (x:xs) f) ;
        Ex x f    -> Ex x  (activateFormula (x:xs) f) 
      }

parseFormulaVar :: [String] -> Parser Formula
parseFormulaVar xs = do { f <- parseFormula0 ; return (activateFormula xs f) } 

parseFormula :: Parser Formula
parseFormula = parseFormulaVar [] 
